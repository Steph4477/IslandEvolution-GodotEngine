...
@export var lines = [
	"C'est bien ! Tu as réussi à aider Toucan", 
	"Tu as tes 6 graines !",
	"Par contre, bon courage avec ...", 
	"La dernière fois ...",
	"Elle m'a volé mon collier d'os ! ...",
]
	...
	if writing:
		...
		if step > 0:
			...
			if shown_chars >= full_line.length():
				...
				writing = false
				$AnimationPlayer.play("laugh")           # ← stop quand la phrase est finie
